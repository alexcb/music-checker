			   ...
			   mpg123_string* path )
{
	...
	if( url->p[pos] == ':' ) {
		...
		if( port )
			ret = mpg123_set_substring( port, url->p, pos, pos2 - pos );
		pos = pos2;
	}
	else if( port )
		ret = mpg123_set_string( port, "80" );
	...
	if( path )
		ret = url->p[pos] == 0 ? mpg123_set_string( path, "/" )
...
#	define INADDR_NONE 0xffffffff
	...
	int sock = -1;
	if( param.verbose > 1 )
		fprintf( stderr, "Note: Attempting old-style connection to %s\n", host->p );
	...
	if( !isip ) { /* Name lookup. */
		if( !( myhostent = gethostbyname( host->p ) ) )
			return -1;
...
#	ifdef HAVE_GAI_ADDRCONFIG
	hints.ai_flags |=
		AI_ADDRCONFIG; /* Only ask for addresses that we have configured interfaces for. */
#	endif
